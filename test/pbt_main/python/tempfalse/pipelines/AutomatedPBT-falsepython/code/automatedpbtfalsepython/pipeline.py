...
def main():
    ...
    spark.conf.set("prophecy.metadata.pipeline.uri", "pipelines/AutomatedPBT-falsepython")
    registerUDFs(spark)

    try:
        
        MetricsCollector.start(spark = spark, pipelineId = "pipelines/AutomatedPBT-falsepython", config = Config)
    except :
        
        MetricsCollector.start(spark = spark, pipelineId = "pipelines/AutomatedPBT-falsepython")
